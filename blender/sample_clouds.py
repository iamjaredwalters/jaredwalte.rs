...
def gather_triangles(root, front_only=True, back_limit=0.25):
    ...
    for ob in root.children_recursive:
        ...
        ev.to_mesh_clear()
    return np.concatenate(verts), np.concatenate(cols), np.concatenate(weights)


def sample_surface(tris, cols, weights, count, seed):
...
def write_cloud(path, pts, cols):
    ...
    header = b"PCLD" + struct.pack("<II", 1, len(pts)) + struct.pack("<fff", *extent.astype(float))
...
def export_cloud(root_name, filename, count=COUNT, seed=7):
    ...
    tris, cols, weights = gather_triangles(root)
    pts, pcols = sample_surface(tris, cols, weights, count, seed)
    n, extent = write_cloud(f"{OUT_DIR}/{filename}", pts, pcols)
